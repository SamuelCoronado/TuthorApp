  ...
  useEffect(() => {
    ...
    return (
        ...
        <div className="card-body">
          ...
          <p>&nbsp;<Link to={`/users/${userId}`}> <strong>{studentName}</strong></Link>&nbsp;  {days === 0? 'today': days === 1? 'a day ago': `${days} days ago` }</p><br />
...
export const TutorComment = ({profileImage, tutor, tutorName, session, sessionName, opinion, rating, date}) => {
  ...
  return (
    <div className="card my-3 bg-muted" style={{width: '800px',  backgroundColor: '#84a9ac'}}>
    <div className="card-body">
      ...
      <p>&nbsp;<Link to={`/users/${userId}`}> <strong>{tutorName}</strong></Link>&nbsp;  {days === 0? 'today': days === 1? 'a day ago': `${days} days ago` }</p><br />
      <p>&nbsp;<strong>{sessionName}</strong>&nbsp; <Rating name="stars" value={rating} precision={1} readOnly/></p>
      <p className="p-2 border border-white rounded">{opinion}</p>
    </div>
  </div>
